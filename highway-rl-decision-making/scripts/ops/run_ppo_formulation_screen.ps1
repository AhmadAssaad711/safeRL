...
if (-not $Resume -and (Test-Path -LiteralPath $OutputDir)) {
    ...
    if ($Existing.Count -gt 0) {
        throw "Fresh formulation-screen output is not empty: $OutputDir"
    }
}

$ScreenArgs = @(
    '-m', 'scripts.training.run_ppo_formulation_screen',
    '--stage', 'screen',
    '--configs',
        'P0_current',
        'P1_reward',
...
$Stdout = Join-Path $LogDir "ppo_formulation_${OutputIdentity}_$Stamp.stdout.log"
$Stderr = Join-Path $LogDir "ppo_formulation_${OutputIdentity}_$Stamp.stderr.log"
$LatestManifest = Join-Path $LogDir "ppo_formulation_${OutputIdentity}.latest.json"

$ProcessArgs = @($ScreenArgs)
