param(
    [string]$OutputDir = 'C:\agv_ppo_formulation_50k',
    [switch]$Watch,
...
function Get-FormulationScreenStatus {
    param([string]$Path)

    $ScriptNeedle = 'scripts.training.run_ppo_formulation_screen'
    $Matching = @(
    ...
    $LatestManifest = Join-Path $LogDir "ppo_formulation_${OutputIdentity}.latest.json"
...
do {
    $Status = Get-FormulationScreenStatus -Path $OutputDir
    $Status | Select-Object Timestamp, InvocationCount, InvocationRootPids,
        ...
        LaunchAgeSeconds, StderrBytes, StdoutLog, StderrLog | Format-List
...
if ($Status.RunFailed) {
    Write-Error 'PPO formulation process exited without ranking_final_three.csv. Inspect the reported logs.'
}
