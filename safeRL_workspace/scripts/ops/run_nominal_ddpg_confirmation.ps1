...
if (-not $Resume -and (Test-Path -LiteralPath $OutputDir)) {
    ...
    if ($existing.Count -gt 0) {
        throw "Fresh confirmation output is not empty: $OutputDir"
    }
}

$PilotArgs = @(
    '-m', 'scripts.training.run_nominal_ddpg_parameter_pilot',
    '--stage', 'confirm',
    '--selected-configs', 'P0_current', 'P2_more_exploration',
    '--seeds', '307', '1307', '2307',
    '--timesteps', '150000',
...
$Stdout = Join-Path $LogDir "confirm_${OutputIdentity}_$Stamp.stdout.log"
$Stderr = Join-Path $LogDir "confirm_${OutputIdentity}_$Stamp.stderr.log"
$LatestManifest = Join-Path $LogDir "confirm_${OutputIdentity}.latest.json"

# Start-Process needs explicit quoting for user-supplied paths containing spaces.
...
[pscustomobject]@{
    ...
    Resume = [bool]$Resume
    Note = 'Two matching python.exe PIDs are normal: the venv launcher plus its child interpreter.'
}
