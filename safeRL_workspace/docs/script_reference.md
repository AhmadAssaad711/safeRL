...
- Safety: HOCBF constraints are evaluated in physical action space and then
  mapped back to the policy action space. The QP uses `(k1, k0) = (8.5, 4.0)`,
  the valid cascade with class-K rates `(c1, c2) = (0.5, 8)`. Spawn, reset,
  and occupancy monitoring use the separate guard `psi1 = h_dot + 2.3 h`.
